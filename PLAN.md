...
| feat | `04-17-feat_logger_timestamp` | Add timestamp prefix to all log output | ✅ PR #244 |
| feat | `04-17-feat_exclude_pages` | Exclude pages from export: `excludePages` field in `.fpdf.json` metadata; UI toggle per-page; exporter skips excluded pages | ✅ PR #245 |
| feat | `04-17-feat_menu_font_size` | Increase context-menu font size to 16 px | ✅ PR #247 |
| fix | `04-17-fix_quote_log_paths` | Quote all file-path references in log messages | ✅ PR #249 |
| docs | `04-17-docs_npm_publish_plan` | Add `NPM_PUBLISH_PLAN.md` outlining scoped npm publish rollout (`@the-hcma/fpdf`) | ✅ PR #251 |
| fix | `04-17-fix_force_black_font_color` | Force black font color in exported AcroForm text field appearances | ✅ PR #263 |
| fix | `04-17-fix_default_left_alignment` | Default to left alignment when no explicit `textAlign` is set on export | ✅ PR #264 |
| fix | `04-17-fix_png_alpha_transparency` | Add page transparency group when drawing PNG images with alpha channel | ✅ PR #265 |
| fix | `04-21-fix_setup-service_restart` | `setup-service`: restart service when built code changes (previously only checked if running) | ✅ PR #262 |
| fix | `04-21-fix_setup-service_git_rev` | `setup-service`: use `git rev-parse HEAD` instead of `mtime` to detect stale dist; more reliable across copies and deployments | ✅ PR #267 |
| feat | `04-21-feat_version_badge` | Version badge in web UI header showing package version + short git commit SHA; placed-image `Multiply` blend mode for transparent stamps | ✅ PR #269 |
| feat | `04-21-feat_black_default_color` | Black as default text color in UI; text-only alignment/color context menus (hidden for non-text fields) | ✅ PR #270 |
| ci | — | Replace broken actor-guard `if:` condition with a `guard` job that calls the GitHub API at runtime (`gh pr view --json state`) to skip CI only when a PR is already merged or the branch is a merge-queue branch | ✅ |
| fix(dev) | `04-21-fix_start-development` | `start-development`: detect primary repo via `worktree list` (not `rev-parse --show-toplevel` which breaks inside linked worktrees); use subshell `cd` instead of `gt --cwd`; `rm -rf` after `git worktree remove` for non-empty dirs; orphaned branch pruning step deletes branches with merged/closed PRs or with no PR and fully merged into `main` | ✅ |
