import { tmpdir } from 'node:os';
import { writeFile, mkdir } from 'node:fs/promises';
import * as path from 'node:path';
import { PDFDocument } from 'pdf-lib';
import type { CandidateField } from '../types.js';

export async function makePdfBytes(
  setup: (doc: PDFDocument) => Promise<void> | void,
): Promise<Uint8Array> {
  const doc = await PDFDocument.create();
  await setup(doc);
  return doc.save();
}

export async function writeTempPdf(
  name: string,
  bytes: Uint8Array,
  subdir = 'fpdf-tests',
): Promise<string> {
  const dir = path.join(tmpdir(), subdir);
  await mkdir(dir, { recursive: true });
  const filePath = path.join(dir, name);
  await writeFile(filePath, bytes);
  return filePath;
}

// Minimal valid 1×1 white JPEG
export const MINIMAL_JPEG = new Uint8Array([
  ,
]);

export function makePlacement(
  x: number,
  y: number,
  width: number,
  height: number,
): { placement: { x: number; y: number; width: number; height: number } } {
  return { placement: { x, y, width, height } };
}

// Minimal valid 1×1 white PNG (RGB, no alpha)
export const MINIMAL_PNG = new Uint8Array([
  0x89, 0x50, 0x4e, 0x47, 0x0d, 0x0a, 0x1a, 0x0a, 0x00, 0x00, 0x00, 0x0d, 0x49, 0x48, 0x44, 0x52,
  0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x01, 0x08, 0x02, 0x00, 0x00, 0x00, 0x90, 0x77, 0x53,
  0xde, 0x00, 0x00, 0x00, 0x0c, 0x49, 0x44, 0x41, 0x54, 0x08, 0xd7, 0x63, 0xf8, 0xcf, 0xc0, 0x00,
  0x00, 0x00, 0x02, 0x00, 0x01, 0xe2, 0x21, 0xbc, 0x33, 0x00, 0x00, 0x00, 0x00, 0x49, 0x45, 0x4e,
  0x44, 0xae, 0x42, 0x60, 0x82,
]);

// Minimal valid 1×1 fully transparent PNG (RGBA, color type 6) — used to test
// that the exporter adds a page transparency group when embedding alpha PNGs.
export const MINIMAL_TRANSPARENT_PNG = new Uint8Array([
  0x89, 0x50, 0x4e, 0x47, 0x0d, 0x0a, 0x1a, 0x0a, 0x00, 0x00, 0x00, 0x0d, 0x49, 0x48, 0x44, 0x52,
  0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x01, 0x08, 0x06, 0x00, 0x00, 0x00, 0x1f, 0x15, 0xc4,
  0x89, 0x00, 0x00, 0x00, 0x0b, 0x49, 0x44, 0x41, 0x54, 0x78, 0x9c, 0x63, 0x60, 0x00, 0x02, 0x00,
  0x00, 0x05, 0x00, 0x01, 0x7a, 0x5e, 0xab, 0x3f, 0x00, 0x00, 0x00, 0x00, 0x49, 0x45, 0x4e, 0x44,
  0xae, 0x42, 0x60, 0x82,
]);

// Minimal valid 1×1 fully transparent PNG (Grayscale+Alpha, color type 4) — used to test
// that the exporter handles the grayscale+alpha case with DeviceGray as the group CS.
export const MINIMAL_TRANSPARENT_GRAY_PNG = new Uint8Array([
  0x89, 0x50, 0x4e, 0x47, 0x0d, 0x0a, 0x1a, 0x0a, 0x00, 0x00, 0x00, 0x0d, 0x49, 0x48, 0x44, 0x52,
  0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x01, 0x08, 0x04, 0x00, 0x00, 0x00, 0xb5, 0x1c, 0x0c,
  0x02, 0x00, 0x00, 0x00, 0x0b, 0x49, 0x44, 0x41, 0x54, 0x78, 0x9c, 0x63, 0x60, 0x60, 0x00, 0x00,
  0x00, 0x03, 0x00, 0x01, 0xb8, 0xad, 0x3a, 0x63, 0x00, 0x00, 0x00, 0x00, 0x49, 0x45, 0x4e, 0x44,
  0xae, 0x42, 0x60, 0x82,
]);

export function makeCandidate(
  type: 'text' | 'textarea' | 'checkbox' | 'radio',
  x: number,
  y: number,
  width: number,
  height: number,
  confidence: 'high' | 'medium' | 'low' = 'high',
): CandidateField {
  return {
    id: crypto.randomUUID(),
    type,
    label: '',
    displayName: 'Field',
    placement: { x, y, width, height },
    value: '',
    confidence,
    dismissed: false,
  };
}
