...
| 15.1 | — | **`POST /upload` endpoint:** accepts a multipart PDF upload, writes bytes to `os.tmpdir()/fpdf-<sessionId>/orig.pdf`, runs `analyzePdf()`, transitions the session to fill mode (reuses existing `POST /open` session-state logic). `ServerOptions` gains a `sessionId` token used as the temp-dir name. Temp dir is cleaned up on WebSocket close / `POST /reset` / server shutdown. |
| 15.2 | — | **Picker UX — "Upload from this computer" path:** `pick.html`/`pick.ts` shows two entry points: existing "Browse server files" tree and a new "Open local file" button. The local button uses `<input type="file" accept=".pdf">` (broad compatibility) with a `DataTransfer`/drag-drop target as a secondary affordance. On file selection the bytes are POSTed to `POST /upload`; WS `pdfOpened` message triggers the same fill-mode transition that `/open` already sends. |
| 15.3 | — | **Companion `.fpdf.json` upload:** when the browser selects a local PDF `foo.pdf`, the picker also silently checks for `foo.fpdf.json` in the same directory using the File System Access API (`showOpenFilePicker` with `startIn` of the PDF's parent, falling back to a second `<input type="file">` prompt). If found, the JSON bytes are uploaded alongside the PDF via a second field in the multipart body; the server writes the JSON to the temp dir and resumes the session from it (same logic as `--json` flag). |
| 15.4 | — | **Streaming export download for uploaded files:** `ServerOptions` / session state tracks whether the current PDF originated from an upload. For uploaded sessions, `POST /save-acroform`, `GET /filled-pdf`, and canvas export return the PDF as `Content-Disposition: attachment` bytes instead of writing to disk — the browser's normal download flow handles saving. The server simultaneously downloads the live `.fpdf.json` from the temp dir so the user can re-upload it next time and resume their session (see 15.3). For server-local sessions the existing disk-write path is unchanged. |
| 15.5 | — | **Auto-download of `.fpdf.json` on first field change (uploaded sessions):** when the user makes their first edit in an uploaded session, the UI silently triggers a `GET /session-json` download so they always have the latest JSON on their local machine without needing to remember to export it. A status-bar indicator shows "session file saved locally" after the download. `GET /session-json` returns the in-memory doc as `application/json` with `Content-Disposition: attachment; filename="<stem>.fpdf.json"`. |
